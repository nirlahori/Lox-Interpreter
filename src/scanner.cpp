...
void Scanner::add_token(TokenType type, Object lit){
    std::string lex {source.substr(start, current - start)};
    Token tok(type, lex, std::move(lit), line);
...
void Scanner::expect_string()
{
    ...
    if (!is_at_end()) {
        ...
        Object obj(str_value);
        add_token(TokenType::STRING, std::move(obj));
    }
    else{
...
std::list<Token> Scanner::scan_tokens()
{
    ...
    Token tok(TokenType::END_OF_FILE, "", nullptr, line);
    tokens.push_back(tok);
    return tokens;
}
